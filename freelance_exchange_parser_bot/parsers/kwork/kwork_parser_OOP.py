...
class KWorkParser:
    ...
    def collect_data(self):
        ...
        return [self.parse_card_to_dict(card) for card in cards]
    ...
    def upgrade(self):
        ...
        # Проблема была в операторе in. Из-за некорректного сравнения объектов dict
        # происходило неверное наполнение output_message_data, вследствие чего в выдаче бота
        # появлялись карточки, которых там быть не должно.
        # Проблема потребовала исключения из списка потенциальных причин
        # многопоточность (добавил ненужный await asyncio.sleep(2)
        # после запроса на формирование message) и
        # множественные импорты (внес функционал в состав класса)
        for card in cards:
...
def main():
    ...
    print(kwork_parser.parse_card_to_message(True))
    print(kwork_parser.parse_card_to_message())
    print(kwork_parser.parse_card_to_file())
