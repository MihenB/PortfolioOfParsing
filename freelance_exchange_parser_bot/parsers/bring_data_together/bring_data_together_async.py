from freelance_exchange_parser_bot.parsers.habr.habr_parser_OOP_async import HabrParser
from freelance_exchange_parser_bot.parsers.fl.fl_parser_OOP_async import FlParser
...
class AllData:
    ...
    async def parse_cards_to_message(self, update_only=False):
        result_list = []
        if update_only:
            await self.habr_parser.parse_card_to_message(update_only)
            await self.fl_parser.parse_card_to_message(update_only)
            self.kwork_parser.parse_card_to_message(update_only)
            return
        result_list.extend(await self.habr_parser.parse_card_to_message(update_only))
        result_list.extend(await self.fl_parser.parse_card_to_message(update_only))
...
async def main():
    all_parsers = AllData()
    print(await all_parsers.parse_cards_to_message(True))
    print(await all_parsers.parse_cards_to_file())
    print(await all_parsers.parse_cards_to_message())
